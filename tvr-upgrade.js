...
window.renderDerating = function() {
  ...
  // ALL values from S (Cell Inputs + Project Targets)
  const chem      = (S.c_chem||'LFP').split(' ')[0].toUpperCase();
  const P_peak    = S.t_ppeak    || 80;   // kW — from Project Targets
  const P_cont    = S.t_pcont    || 50;   // kW
  const T_cell_max= S.t_tcell_max|| 55;   // °C — cutoff
  const T_derate  = T_cell_max - 10;      // start derate 10°C below limit
  const T_op_lo   = S.t_top_lo  ?? -20;   // min operating temp
  const T_op_hi   = S.t_top_hi  ?? 45;    // max ambient
  const V_nom     = S.V_nom_pack || 400;  // V
  const V_max     = S.V_max_pack || 420;
  const V_min     = S.V_min_pack || 280;
  const ir_bol    = S.c_ir_bol   || 0.22; // mΩ cell
  const S_total   = S.S_total    || 112;
  const c_pp      = S.c_pp       || 1;
  const c_ah      = S.c_ah       || 120;
  const c_ir_eol  = S.c_ir_eol   || 0.35;
