...
window.renderSizing = function() {
  ...
  /* ── S range: voltage constraints + always include current Cell Inputs config ── */
  const S_min = Math.ceil(t_V_min / c_vmax);
  const S_max = Math.floor(t_V_max / (c_vnom * 1.01));
  const s_lo = Math.max(1,   Math.min(S_min - 5, curr_Ss - 15));
  const s_hi = Math.min(300, Math.max(S_max + 5, curr_Ss + 15));
  const S_range = [];
  for (let s = s_lo; s <= s_hi; s++) S_range.push(s);
  if (!S_range.includes(curr_Ss)) S_range.push(curr_Ss);
