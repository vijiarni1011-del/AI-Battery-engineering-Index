...
window.renderCellQual = function() {
  ...
  const cell = {
    ...
    vnom:    +(document.getElementById('cq_c_vnom')?.value  ?? S.c_vnom  ?? 3.65),
    vmax:    +(document.getElementById('cq_c_vmax')?.value  ?? S.c_vmax  ?? 4.20),
    vmin:    +(document.getElementById('cq_c_vmin')?.value  ?? S.c_vmin  ?? 2.80),
    ah:      +(document.getElementById('cq_c_ah')?.value    ?? S.c_ah    ?? 120),
    ir_bol:  +(document.getElementById('cq_ir_bol')?.value  ?? S.c_ir_bol ?? 0.22),
    ir_eol:  +(document.getElementById('cq_ir_eol')?.value  ?? S.c_ir_eol ?? 0.35),
    mass_g:  +(document.getElementById('cq_mass')?.value    ?? S.c_mass  ?? 2800),
    cp:      +(document.getElementById('cq_cp')?.value      ?? S.c_cp    ?? 1050),
    t_min:   +(document.getElementById('cq_t_min')?.value   ?? -20),
    t_max:   +(document.getElementById('cq_t_max')?.value   ?? 60),
    c_max_dis:+(document.getElementById('cq_c_dis')?.value  ?? 3.0),
    c_max_chg:+(document.getElementById('cq_c_chg')?.value  ?? 1.5),
    cycles:  +(document.getElementById('cq_cycles')?.value  ?? 2000),
    soh_eol: +(document.getElementById('cq_soh_eol')?.value ?? 80),
  ...
  const req = {
    vnom_min:  S.t_vmin_sys  || 280,
    vnom_max:  S.t_vmax_sys  || 450,
    e_min:     S.t_emin      || 40,
    p_peak:    S.t_ppeak     || 80,
    p_cont:    S.t_pcont     || 50,
    t_op_lo:   S.t_top_lo    ?? -20,
    t_op_hi:   S.t_top_hi    ?? 55,
    t_cell_max:S.t_tcell_max ?? 60,
    c_peak:    S.t_cpeak     || 2.0,
    c_chg:     S.t_cchg      || 0.5,
    ...
    Ss:        S.S_total     || (S.c_cps||14)*(S.c_ss||8) || 112,
    Pp:        S.c_pp        || 1,
  };
  ...
  <div class="card">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px">
      ...
      <div><label class="cq-lbl">Pack Config (auto)</label>
        <div style="padding:6px 8px;background:var(--bg4);border:1px solid var(--border);border-radius:6px;font-family:var(--mono);font-size:12px;color:var(--blue2)">${req.Ss}S / ${req.Pp}P</div></div>
    </div>
