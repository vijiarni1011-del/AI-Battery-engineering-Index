...
const BT_CATS = [
  ...
  { id:'standards',     icon:'📜', label:'Standards Ref',      color:'#60a5fa' },
];
...
function bt_ctx() {
  ...
  return {
    ...
    sxp:    `${S.S_total||112}S/${g('c_pp')||S.c_pp||1}P`,
...
window.renderBatteryTesting = function(catId) {
  ...
  const ctxItems = [
    ...
    ['V_max',    (+ctx.vmax).toFixed(0)+'V'],
    ['V_min',    (+ctx.vmin).toFixed(0)+'V'],
    ['Vc_max',   (+ctx.vcell_max).toFixed(3)+'V'],
    ['Vc_min',   (+ctx.vcell_min).toFixed(3)+'V'],
    // Power
    ['P_peak',   ctx.ppeak+'kW'],
    ['P_cont',   ctx.pcont+'kW'],
    ['I_peak',   ctx.imax+'A'],
...
function bt_emi_emc(c) {
  return `
<div class="bt-section-title">📡 EMI/EMC Test Parameters</div>
<div class="card" style="margin-bottom:16px">
...
function bt_certification(c) {
  ...
  return `
<div class="bt-section-title">🏅 Required Certifications — ${mkts}</div>
...
<div class="bt-section-title">📋 UN 38.3 Test Sequence (mandatory all markets)</div>
<div class="bt-cols">
